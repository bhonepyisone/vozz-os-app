...
"use client";
// This file remains the same as Part 9, but ensure it's up to date.
...
export default function ManagementPage() {
  ...
  const handleInvite = async (e) => {
    ...
    const isAlreadyMember = shop.roles.some(role => role.email === inviteEmail);
    if (isAlreadyMember) {
    ...
    try {
      ...
      // Also update the shop's roles array for display purposes
      const shopRef = doc(db, 'shops', shop.id);
      await updateDoc(shopRef, {
        roles: arrayUnion({ email: inviteEmail.toLowerCase().trim(), role: inviteRole, uid: null })
      });
  ...
  if (isLoading) {
    return <p>Loading Management Area...</p>;
  }
